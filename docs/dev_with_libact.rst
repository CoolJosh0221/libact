...
three methods to be implmented:
:py:meth:`libact.base.interfaces.Model.train`,
:py:meth:`libact.base.interfaces.Model.predict`, and
:py:meth:`libact.base.interfaces.Model.score`. For learning models that supports
continuous output, method
:py:meth:`libact.base.interfaces.ContinuousModel.predict_real` should be
implemented for :code:`ContinuousModel`.
...
The
:py:meth:`libact.query_strategies.active_learning_by_learning.ActiveLearningByLearning.update`
...
retrieved label. Take ALBL's update method as example:

.. literalinclude:: ../libact/query_strategies/active_learning_by_learning.py
   :language: python
   :pyobject: ActiveLearningByLearning.update

:py:meth:`libact.query_strategies.uncertainty_sampling.UncertaintySampling.make_query`
...
output from
:py:meth:`libact.base.interfaces.ContinuousModel.predict_real` of a
...
:py:class:`libact.base.interfaces.Labeler` and implment the
:py:meth:`libact.base.interfaces.Labeler.label`
function with how to retrieve the label of a given sample (feature).

Examples
^^^^^^^^
We have provided two example labelers:
:py:class:`libact.labelers.ideal_labeler.IdealLabeler` and
:py:class:`libact.labelers.interactive_labeler.InteractiveLabeler`.

:py:class:`libact.labelers.ideal_labeler.IdealLabeler` is usually used for
testing the performance of a active
learning algorithm. You give it a fully-labeled dataset, simulating a oracle
that know the true label of all samples. Its
:py:meth:`libact.labelers.ideal_labeler.IdealLabeler.label` is simple
...
:py:class:`libact.labelers.interactive_labeler.InteractiveLabeler` can be used
in the situation where you want to
show your feature through image, let a human be the oracle and label the image
interactively. To implement its
:py:meth:`libact.labelers.interactive_labeler.InteractiveLabeler.label` method,
it may include showing the feature through image using
``matplotlib.pyplot.imshow`` and receive
